...
/*
 * This struct is to be used to check if a request is a locking request and
 * if yes then check if the lock type is F_UNLCK. In this case the request
 * should be handled by the main virtqueue thread or the GThreadPool to avoid
 * deadlocks. Deadlocks can happen when all the custom threads are blocked
 * waiting for locks
 */
struct fv_LockReqInfo {
    int opcode;                                    /* Lock request opcode */
    bool unlock;                                      /* F_UNLCK lock? */
};
...
static struct fv_LockReqInfo *fv_get_request_info(gpointer data,
                                            gpointer user_data)
{
    struct fv_QueueInfo *qi = user_data;
    struct fuse_session *se = qi->virtio_dev->se;
    ...
    struct fv_LockReqInfo *req_info;
    ...
    req_info = g_new(struct fv_LockReqInfo, 1);
    /* Get the opcode from the header */
    req_info->opcode = inh.opcode;
    /* Set the unlock flag to false for now */
    req_info->unlock = false;

    /*
     * We also need to get the body of the message
     * In case of a locking request we will check if the lock is of
     * type F_UNLCK and thus it must not be serviced by a thread of the
     * custom threadpool
     */
    copy_from_iov(&fbuf, out_num, out_sg, se->bufsize);
    /* That copy reread the in_header, make sure we use the original */
    ...
    /* Sharing code from fuse_lowlevel.c */
    iter = FUSE_MBUF_ITER_INIT(buf);
    /*
     * If the request is a FUSE_SETLKW request then we must check if the
     * type of the lock is F_UNLCK. If yes then the request should be
     * serviced by the main thread and not a thread from the custom
     * threadpool
     */
    if (inh.opcode == FUSE_SETLKW) {
        struct fuse_lk_in *arg;
        /*
         * First advance to the end of the in header and the start of the
         * payload
         */
        fuse_mbuf_iter_advance(&iter, sizeof(inh));
        arg = fuse_mbuf_iter_advance(&iter, sizeof(*arg));
        /*
         * TODO: Handle the case where the payload buffer is not setup
         * properly
         */
        if (arg->lk.type == F_UNLCK) {
            req_info->unlock = true;
        }
    }
...
static void *fv_queue_thread(void *opaque)
{
    ...
    while (1) {
        ...
        while (1) {
             ...
             */
            req_info = fv_get_request_info(req, qi);
            if (!se->thread_pool_size) {
                if (req_info->opcode == FUSE_SETLKW && !req_info->unlock) {
...
static void fv_queue_cleanup_thread(struct fv_VuDev *vud, int qidx)
{
    ...
    if (qidx != 1) {
        ...
        /*
         * When the request virtqueue exits also wait for the inotify thread to
         * exit safely
         */
        if (qidx == 2) {
            while (1) {
