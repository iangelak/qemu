...
static void fv_queue_cleanup_thread(struct fv_VuDev *vud, int qidx)
{
    ...
    if (!se->notify_enabled || qidx != 1) {
        ...
        close(ourqi->kill_fd);
    } else if (se->notify_enabled && se->inotify && (qidx == 0)) {
        /* Signal the inotify thread to cleanup and exit */
        pthread_mutex_lock(&se->inotify->i_lock);
        se->inotify->cleanup_inotify = 1;
        pthread_mutex_unlock(&se->inotify->i_lock);
        /* Wait for the inotify thread to exit safely */
        while (1) {
            pthread_mutex_lock(&se->inotify->i_lock);
            running = se->inotify->thread_running;
            pthread_mutex_unlock(&se->inotify->i_lock);
            if (!running) {
                break;
            }
